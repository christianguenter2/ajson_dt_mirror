...
CLASS lcl_empty_filter IMPLEMENTATION.
  METHOD zif_dt_ajson_filter~keep_node.
    ...
    rv_keep = boolc(
      ( iv_visit = zif_dt_ajson_filter=>visit_type-value AND is_node-value IS NOT INITIAL ) OR
      ( iv_visit <> zif_dt_ajson_filter=>visit_type-value AND is_node-children > 0 ) ).
...
CLASS lcl_paths_filter IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF boolc( iv_skip_paths IS INITIAL ) = boolc( it_skip_paths IS INITIAL ). " XOR
      zcx_dt_ajson_error=>raise( 'no filter path specified' ).
    ENDIF.
...
CLASS lcl_and_filter DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        it_filters TYPE zif_dt_ajson_filter=>ty_filter_tab
      RAISING
        zcx_dt_ajson_error.
  PRIVATE SECTION.
    DATA mt_filters TYPE zif_dt_ajson_filter=>ty_filter_tab.
ENDCLASS.
